...
class _PaymentScreenState extends ConsumerState<PaymentScreen> {
  ...
  static const String _backendUrl =
      'https://cozily-unfulminating-amia.ngrok-free.dev';
  ...
  void _showSuccessAndClose(String message) async {
    ...
    try {
      if (widget.ticket.id != null) {
        ...
        debugPrint(
            "Client-side update successful for ticket ${widget.ticket.id}");
      }
    } catch (e) {
      debugPrint("Lỗi client-side update ticket status: $e");
    }
  ...
  Future<void> _startPayment() async {
    setState(() {
      ...
      _qrData = null;
      _statusMessage =
          "Đang khởi tạo thanh toán qua ${widget.paymentMethod}...";
    });
  ...
  Future<void> _handleMoMoPayment() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData['isFree'] == true) {
        debugPrint('Vé miễn phí, đã xác nhận từ backend.');
        // Backend đã xử lý xong, chỉ cần hiển thị thành công
        _showSuccessAndClose('Thanh toán bằng voucher thành công!');
      ...
      if (deepLink != null && deepLink.isNotEmpty) {
        ...
        debugPrint('Backend không trả về deep link!');
      }
    ...
    _pollingTimer = Timer.periodic(const Duration(seconds: 5), (timer) async {
      ...
      try {
        final checkResponse = await http.get(
          Uri.parse(
              '$_backendUrl/api/payment/check-status/${widget.ticket.id}'),
        );
        if (checkResponse.statusCode == 200) {
          ...
          if (data['isPaid'] == true) {
            ...
            timer.cancel();
            _showErrorAndRetry('Thanh toán thất bại. Vui lòng thử lại.',
                canRetry: true);
          }
        }
      } catch (e) {
        debugPrint('Lỗi khi polling: $e');
      }
    });
  }
              ...
              children: [
                if (_isLoading) ...[
                  ...
                  Image.asset(
                    ...
                    height: 80.h,
                    errorBuilder: (c, e, s) =>
                        Icon(Icons.payment, size: 80.h, color: Colors.grey),
                  ),
